...
3. **Board create/delete on desktop** — ✅ **resolved (issue #10).** The issue is silent, yet both currently live in surfaces desktop removes (list-view button; long-press on rows) — without new affordances desktop cannot create or remove boards at all. Confirmed in placement (both live in the pane, reusing the existing `deleteBoard` 5 s-undo flow), sharpened in form, because the default left both controls' *shape* unexamined:
   ...
   - **Create is a ghost card, not a filled button.** Reusing mobile's `#new-board` would put the visually heaviest object in the app — a solid `--ink` fill — at the top of a *sunken, darker* rail, the control outweighing the boards it makes. Instead the control is an empty card at the head of the stack at 2 px `--ink-shadow`: verbatim the `.tap-ghost` language of **B18(c)** — *"the frame the tap will produce, drawn first at its lightest weight."* Because pane order is `createdAt` descending (decision 7), the affordance sits exactly where its result will appear, so the 400 ms window fills by the ghost **becoming** the card rather than by a button flashing elsewhere on screen. A real `<button>` labelled "New board" (keyboard/AT unaffected), acknowledging by filling to `--ink` weight — the `#new-board.tapped` *drain* precedent exists only because that button is already filled.
   - **Delete is revealed on hover/focus over any card, in the menu's own grammar.** Any board is deletable without opening it, which is what mobile's list already allows; restricting delete to the active card would remove a capability rather than translate one. The control is **A1**'s menu row compressed into the card's right edge — `GLYPH.delete` in `--danger`, hairline-separated from the title, `.tapped` fill — so it reads as the same act as the mobile menu item, not a new one. Revealed on `:hover` **and** `:focus-within` so keyboard reaches it; its width is reserved in the card's layout at all times, so revealing it never reflows the title; its own hit target, so it can never be mistaken for the card's swap click.
   - **No confirm step; Undo pays for it.** One click → the 400 ms acknowledged window (B18) → `deleteBoard` → 5 s Undo toast. Desktop loses mobile's 500 ms long-press deliberateness, and Undo is what buys it back — as it does for every other destruction in this app.
   - **Impermanence:** the ghost card holds until the pane is exercised at a real board count; if a rail of 12+ boards scrolls it out of reach, the control pins instead of scrolls — the language survives, only its position moves. Hover-reveal holds because `(hover: hover)` gates the mode (decision 1), so a hoverless wide screen never reaches this code; a hybrid touch-laptop in real use would want the control always visible rather than revealed.
...
- Create/delete affordances, per resolved decision 3: the **New board ghost card** is the first child of the card stack — same card geometry, 2 px `--ink-shadow` border, no fill (the `.tap-ghost` rule generalizes; its `position:absolute`/44 px sizing does not, so the card takes the border/radius as a shared `.ghost` weight rather than reusing the class). It acknowledges by filling to `--ink` weight, and the record it creates renders directly beneath it. The **delete glyph** is the last child of each card, `--danger` on `GLYPH.delete`, hairline-separated, in a permanently reserved 44 px slot (`opacity` toggled on `.pane-card:hover`, `.pane-card:focus-within` — never `display`, so nothing reflows) and a sibling `<button>` rather than nested in the card's own button, so the card stays one clean swap target and the glyph is its own tab stop. Both route through `delayAction` (B18 intact for actions).
