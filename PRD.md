...
`styles.css`, `app.js`, `manifest.json`, `sw.js`, plus `icons/` and (per §9.2)
`fonts/`.
...
hairline** — and, per `UIUX §1.4`, never distinguished by colour alone. The
...
- **The export is always paper-light**, whatever the app looks like (§9.4). Its
  palette is named separately from the screen's in `UIUX §2.8`.
...
**Depth reads as literal darkness.** Surfaces are ordered by luminance, and
nothing is distinguished from another surface by hue alone. The band recedes
because it is structure you read past. The Parking Lot is lit because it is a
shelf things rest on. The note is the brightest surface in the app because it is
the only thing the person made — if a re-tune ever made some other surface
brightest, the identity would be wrong even if every contrast ratio still
passed.

**One warm hue, and it is the destructive one.** Everything else is cool water;
the one thing that destroys is the one thing that isn't. It need not shout —
position and the hairline above it already carry the meaning (§6.6).
...
(§3.2), so it ships in `fonts/` and is subject to §8.1's bump.
...
| Looking for | `UIUX.md` |
|---|---|
| the surface ladder and the law that generates it | §2.1 |
| the ink poles, the crossover, the forbidden band | §2.2 |
| accents, lines, the focus ring | §2.3 |
| elevation | §2.4 |
| typography, the size scale, glyphs | §2.5 |
| space, size, radius, border weight | §2.6, §2.7 |
| the export's paper palette | §2.8 |
| migrating v1's thirteen tokens | §2.9 |
...
named separately in `UIUX §2.8`, so retiring the light theme does not leave the
...
1. **Five sync points, none automated** — enumerated in `UIUX §2.9.2`.
...
3. **`EXPORT_GEO.radius` does not move.** An earlier draft of §9 took the note's
   radius from 2px to 3px. It stays at 2 (`UIUX §2.7`): the radius set is three
   steps — 2 drawn, 3 the selection ring, 8 elevated transient — and the ring is
   3 *because* it sits 1px outside a 2px note. Moving the note to 3 forces the
   ring to 4 and buys nothing; 2px already reads as drawn at every scale a note
   can take. Same class of unexamined import as the `font-color` and 10px label
   the control spec already corrected.
...
5. The `prefers-color-scheme: dark` block and the entire light `:root` are
   removed, not overridden. `UIUX §2.9` gives every one of v1's thirteen tokens
   a fate, and `UIUX §2.9.1` the ground → ink binding for the 60 call sites the
   two retiring poles carry.
6. **The band's type metrics must be re-verified against the shipped font files**
   before release (`UIUX §2.5.2`). The band is sized by the type it holds (B37)
...
7. **Three JS duration constants are paired to CSS values** and must move
   together (`UIUX §8.2`); nothing tests the pairing.

---

## §10 The boundary with `UIUX.md`

`UIUX.md` now exists, at the `§1`–`§12` numbering the codebase already cites in
37 places and that `styles.css:4-5` carries as its own header index. **Every
...
`UIUX.md`'s Appendix A mandates the document prefix on every new citation:
...
- **`PRD §6.2`**'s predetermined width cap was superseded by B39 — a note wraps
  at the sheet's right edge (§6.2, `UIUX §4.2.1`).
